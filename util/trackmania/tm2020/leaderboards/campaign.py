...
def get_player_good_maps(player_name: str) -> discord.Embed:
    ...
    player_embed = ezembed.create_embed(title=f'{player_name} is good at the following maps', color=discord.Colour.random())
    top_100_string = ''
    top_200_string = ''
    top_300_string = ''
    top_400_string = ''
        ...
        for player_tuple in player_list:
            if player_tuple[0].lower() == player_name.lower():
                if int(player_tuple[1]) <= 100:
                    ...
                    top_100_string = top_100_string + str(i) + ' - ' + str(player_tuple[1]) + '\n'
                elif int(player_tuple[1]) <= 200 and int(player_tuple[1]) > 100:
                    log.debug(f'{player_name} is a top 200 player for Map {i}')
                    top_200_string = top_200_string + str(i) + ' - ' + str(player_tuple[1]) + '\n'
                elif int(player_tuple[1]) <= 300 and int(player_tuple[1]) > 200:
                    log.debug(f'{player_name} is a top 300 player for Map {i}')
                    top_300_string = top_300_string + str(i) + ' - ' + str(player_tuple[1]) + '\n'
                elif int(player_tuple[1]) <= 400 and int(player_tuple[1]) > 300:
                    log.debug(f'{player_name} is a top 400 player for Map {i}')
                    top_400_string = top_400_string + str(i) + ' - ' + str(player_tuple[1]) + '\n'
                elif int(player_tuple[1]) <= 500 and int(player_tuple[1]) > 400:
                    log.debug(f'{player_name} is a top 500 player for Map {i}')
                    top_500_string = top_500_string + str(i) + ' - ' + str(player_tuple[1]) + '\n'
                    
    
    if top_100_string != '':
        log.debug(f'Appending Top 100 String for {player_name}')
        player_embed.add_field(name='**Top 100**', value='```' + top_100_string + '```', inline=False)
    else:
        log.debug(f'Player does not have any top 100 ranks')
        player_embed.add_field(name='**Top 100**', value='Player does not have any top 100 times for maps 06-25', inline=False)         
                    
    if top_200_string != '':
        log.debug(f'Appending Top 100 String for {player_name}')
        player_embed.add_field(name='**Top 200**', value='```' + top_200_string + '```', inline=False)
    else:
        log.debug(f'Player does not have any top 200 ranks')
        player_embed.add_field(name='**Top 200**', value='Player does not have any top 200 times for maps 06-25', inline=False)         

    if top_300_string != '':
        log.debug(f'Appending Top 100 String for {player_name}')
        player_embed.add_field(name='**Top 300**', value='```' + top_300_string + '```', inline=False)
    else:
        log.debug(f'Player does not have any top 300 ranks')
        player_embed.add_field(name='**Top 300**', value='Player does not have any top 300 times for maps 06-25', inline=False)         

    if top_400_string != '':
        log.debug(f'Appending Top 100 String for {player_name}')
        player_embed.add_field(name='**Top 400**', value='```' + top_400_string + '```', inline=False)
    else:
        log.debug(f'Player does not have any top 400 ranks')
        player_embed.add_field(name='**Top 400**', value='Player does not have any top 400 times for maps 06-25', inline=False)         

    if top_500_string != '':
        log.debug(f'Appending Top 100 String for {player_name}')
        player_embed.add_field(name='**Top 500**', value='```' + top_500_string + '```', inline=False)
    else:
        log.debug(f'Player does not have any top 500 ranks')
        player_embed.add_field(name='**Top 500**', value='Player does not have any top 500 times for maps 06-25', inline=False)         
