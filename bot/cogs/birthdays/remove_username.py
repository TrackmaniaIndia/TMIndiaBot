...
class RemoveBirthday(commands.Cog):
    ...
    @commands.slash_command(guild_ids=constants.Bot.default_guilds, name='remove_username', description='Remove a user\'s birthday with their discord id')
    @permissions.has_any_role("Moderator", "Admin", "Bot Developer", "Developer")
    async def _remove_username(self, ctx: commands.Context, id: Option(int, 'The user\'s id', required=True)):
        ...
        if mod_log_channel is not None:
            await mod_log_channel.send(f'Requestor: `{ctx.author.name}` is requesting removal of `{id}`\'s birthday')
        ...
        if success_flag:
            ...
            await ctx.respond(f'Successfully removed birthday of `{id}`', ephemeral=True)
        if not success_flag:
            log.debug(f'This user {id} did not have a birthday saved')
            await ctx.respond(f'This User `{id}` does not have a birthday saved', ephemeral=True)
